...
describe('file', () => {
    it('001 repository file', async () => {
        ...
        let file = await app
            .httpRequest()
            .get(
                `/api/v0/file?filepath=${filepath}&repopath=${repopath}&commitId=${commitId}`
            )
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 404))
            .catch(_ => {});
    ...
    it('002 upload commit', async () => {
        ...
        const sha1 = await app
            .httpRequest()
            .post('/api/v0/file/upload')
            .send({
                repopath,
                encoding: 'base64',
        ...
        let commitId = await app
            .httpRequest()
            .post('/api/v0/file/commit')
            .send({
                repopath,
                files: [
                    {
        ...
        const list = await app
            .httpRequest()
            .get(
                `/api/v0/file/history?filepath=test/file3&repopath=${repopath}`
            )
            .set('Authorization', `Bearer ${token}`)
            .expect(res => assert(res.statusCode == 200))
            .then(res => res.body);
    ...
    it('003 tree', async () => {
        ...
        let trees = await app
            .httpRequest()
            .get(`/api/v0/file/tree?repopath=${repopath}`)
            .expect(res => assert(res.statusCode === 200))
            .then(res => res.body);
        ...
        trees = await app
            .httpRequest()
            .get(`/api/v0/file/tree/?repopath=${repopath}&filepath=dir/`)
            .expect(res => assert(res.statusCode === 200))
            .then(res => res.body);
        //console.log(trees);
