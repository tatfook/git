...
function getFiles(path) {
    ...
    fs.readdirSync(path).forEach(
        file => (files = files.concat(getFiles(_path.join(path, file))))
    );
    return files;
}
    ...
    it('001 share world', async () => {
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            await app
                .httpRequest()
                .get(
                    `/api/v0/file/raw?filepath=${filepath}&repopath=${repopath}`
                )
                .set('Authorization', `Bearer ${token}`)
                .expect(res => assert(res.statusCode == 200))
                .then(res => res.body)
                .catch(e => console.log('not fount:--------', filepath));
            //console.log(file);
        }
