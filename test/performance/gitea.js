...
const init = async () => {
	...
	for (let i = 0; i < 1000; i++) {
		const repo = `repo${i}`;
		await axios.post(`${baseUrl}/user/repos`, {
			name: repo,
			"private": true,
			"auto_init": true,
		}, {
...
const run = async () => {
	...
	for (let count = 0; count < 100; count++) {
		...
		for (let i = 0; i < 50; i++) { 
			...
			promises.push(axios[files[path] ? "put" : "post"](`${baseUrl}/repos/xiaoyao/${repo}/contents/${filename}`, {
				shaa: files[path],
				content:"JXU4RkQ5JXU5MUNDJXU2NjJGJXU4OTgxJXU1MkEwJXU1QkM2JXU3Njg0JXU1MTg1JXU1QkI5JXVGRjAx",
				message:"message",
			}, {
				headers: {
					Authorization: `Bearer ${token}`
